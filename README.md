...
It is acomplished using .in and .ans that contain the expected input and output of the program in plain-text format.
The program expects these files to be named `x.in` and `x.ans` where x is the number of the testcase (starting with 1).
This is usefull in scenarios like some compeditive programming where the code needs to pass predefined tests.
...
```
cptester <file to test> <number of testcases>
```
